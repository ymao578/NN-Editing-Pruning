...
def define_loss(y, ly, params, weights, trainable_var):
...
print(tf.__version__) x  -- placeholder for input: x(k)
    ...
    loss1 = tf.reduce_mean(tf.square(y-ly))
    
    ##Compute regulation############################################################
    num_of_layers = params['uncheckable_num_of_layers']
    for i in np.arange(num_of_layers):
        Mat = weights['W%s%d' % (name, i + 1)]    #original weight matrix 
        if i == 0:
            Mat_norm = tf.math.reduce_euclidean_norm(Mat)
        else:
            Mat_norm = Mat_norm + tf.math.reduce_euclidean_norm(Mat)
    #################################################################################
    ...
    loss = params['dynamics_lam']*loss1 + params['regulation_lam']*Mat_norm;
...
def try_exp(params):
    ...
    loss = define_loss(y, ly, params, weights, trainable_var)
    ...
    valy = np.loadtxt(('./trainingdata1/%s_valY.csv' % (params['data_name'])), delimiter=',',
                                        dtype=np.float32)
    #valz = np.loadtxt(('./trainingdata1/%s_valz.csv' % (params['data_name'])), delimiter=',',
    #                                    dtype=np.float32)
