...
def taylor_nn(prev_layer, weights, phyweights, unkweights, biases, inx_phy, inx_unk, com_type1, com_type2, act_type, num_of_layers,                       expansion_order, name ='U'):
    ...
    for i in np.arange(num_of_layers):
        ...
        ##Matrix purnin####################################################################################################################
        orgMat = weights['W%s%d' % (name,i + 1)]    #original weight matrix 
        threshold = tf.constant(0.001, tf.float32)             #threshhold 
        Sow = orgMat.get_shape()                    #get matrix size
        absMat = tf.abs(orgMat)                     #absolute original weight matrix 
        oneMat = tf.ones(Sow, tf.float32)             #ones matrix  
        thrMat = tf.multiply(oneMat, threshold)     #thresholdmatrix
        comMat = tf.greater(absMat, thrMat) 
        indMat = tf.cast(comMat, tf.float32)
        auxMat = tf.multiply(orgMat, indMat)
        ###########################
        
        pre_weight_B = tf.multiply(auxMat, unkweights['W%s%d' % (name,i + 1)])
